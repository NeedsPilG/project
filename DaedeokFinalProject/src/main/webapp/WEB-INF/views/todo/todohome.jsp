				...
				success : function(res){
			    	...
			    	txt += `
            			...
            			`;
            				...
            				if(res.todo.finYn === 'Y'){
            					txt += `
            					<input type="text" class="tdContent\${res.todo.tdNo } no-border-input w-50" id="tdContent${todo.tdNo } " name="tdContent" value="${todo.tdContent }"
            					style="text-decoration: line-through; background-color: transparent; border: none; " disabled="true">
            					`;
            				} else {
            					txt += `
	            					<input type="text" class="tdContent\${res.todo.tdNo } no-border-input w-50" id="tdContent\${res.todo.tdNo }" name="tdContent" value="\${res.todo.tdContent }" 
	            					style="border: none; background-color: transparent; "disabled="true">
            					`;
            				}
            			txt += `
			            ...
			            if(res.todo.importYn === 'Y'){
			            	txt +=`
					          	<span class="d-none" id="importText\${res.todo.tdNo }">중요 여부</span>   <input class="form-check-input d-none" type="checkbox" id="importYn${todo.tdNo }" 
					          	name="importYn" value="\${res.todo.importYn }" style="flex: 0 0 auto;" checked="checked">
			            	`;
			            } else {
			            	txt += `
					          	<span class="d-none" id="importText\${res.todo.tdNo }">중요 여부</span>   <input class="form-check-input d-none" type="checkbox" id="importYn${todo.tdNo }" 
					          	name="importYn" value="\${res.todo.importYn }" style="flex: 0 0 auto;" >
			            	`;
			            }
			           txt +=` 
